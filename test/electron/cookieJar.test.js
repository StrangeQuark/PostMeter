...
test('preserves explicit Cookie header values over jar values', () => {
  ...
  assert.equal(header, 'sid=explicit; theme=dark; newCookie=yes');
});
...
test('parses Set-Cookie defaults and rejects invalid cross-domain cookies', () => {
  ...
  const parsed = parseSetCookie('sid=abc; Domain=.example.test; HttpOnly; SameSite=None', url);
