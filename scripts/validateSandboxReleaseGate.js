...
requireScript('check', [
  ...
  'npm run refactor:structure:validate',
...
for (const scriptName of [
  ...
  'release:validate:packaged-workflow',
  'release:validate:packaged-auth',
...
requireWorkflow('CI workflow', ciWorkflow, [
  ...
  /xvfb-run -a npm run test:ui:hawk/,
  /xvfb-run -a npm run test:ui:aws/,
...
for (const [label, workflow] of [
  ...
  if (/node-version:\s*22/.test(workflow)) {
    errors.push(`${label} must use Node 24.21.0 for all GitHub Actions Node setup steps.`);
  }
  if (/node-version:\s*24(?:\s|$)/.test(workflow)) {
    errors.push(`${label} must pin GitHub Actions Node setup steps to Node 24.21.0, not floating Node 24.`);
  }
