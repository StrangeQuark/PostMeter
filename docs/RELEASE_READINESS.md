...
The desktop Runners UI is now a first-class workspace section rather than a request result-panel tab. Release validation should treat runner data as workspace data: schema-12 workspaces include `workspace.runners`, runner-owned requests are independent copies, runner capture policies are saved with the runner, and runner environment mutations persist only when the runner's mutation option is enabled. Source UI evidence should cover runner creation from the toolbar and empty pane, runner request import/edit/reorder/delete behavior, runner request targeted saves, runner tab dirty-close behavior, capture settings, pre-run result-store size warnings, temp-store-backed result paging, shutdown cleanup of temp result files, high-volume guardrails, and the split runner execution results pane.
...
Performance is tracked as local first-class saved performance tests under `workspace.performanceTests`. The legacy Load Test and JMeter bridge have been removed, and the local V1 Performance implementation now covers the workspace model, migrations, renderer surface, execution engine, local client calibration, IPC, diagnostics boundary, safety caps, import/export validation, scoped one-click Full Endpoint Diagnosis, and eight-type positive/negative scenario coverage.

Release evidence for Performance covers request-copy isolation from Collections, manual request entry, environment copy-vs-mutate behavior, import/export validation, safety cap rejection and cancellation, target-rate local calibration with cancellable loopback execution, capture settings, pre-run result-store size warnings, temp-store-backed result paging/export, shutdown cleanup of temp result files, high-volume capture guardrails, Full Endpoint Diagnosis Quick/Medium/Extended scope planning, transport/behavior/security/client-confidence checks, UAT-friendly diagnostic CSV export, progress reporting, bounded probe-and-verification stages, local latency-knee rejection, empty pane, New dropdown, sidebar placement between Runners and History, dirty tab behavior, and save/discard semantics. The production matrices keep distributed/cloud load execution explicitly out of scope.

Distributed/cloud load execution, hosted agents, account-gated load runs, and JMeter import/export/execution remain outside the current production claim.
...
- UX/accessibility/failure-recovery matrix and validation, including tab and panel ARIA semantics, rerender-safe tab focus, keyboard-operable splitter separators, autocomplete listbox/active-descendant relationships, visible app live status, toolbar/tree/tab/history context-menu keyboard behavior and activation focus restoration after modal rerenders, modal focus trapping/restoration, metadata-only vault prompts including native fallback reset decisions, in-app package/file/workspace/vault/tree/history input or confirmation modals instead of raw native prompt/confirm/alert dialogs, inline request/environment/workspace title editing, pane-local save actions, tab cap/force-close behavior, draggable sidebar ordering with single insertion-bar feedback and structural-only saves, runner import/edit/reorder/delete and split execution-result UI, light/dark/system plus active forced-colors focus behavior, constrained desktop and long-label smoke coverage, package review/fetch/removal and file binding/removal smoke coverage including cancellation and failed-save rollback, vault bind/remove/reset smoke coverage, failed import/export/send/save/OAuth recovery checks, failed pre-send/pre-run save recovery, stale runner completion rejection after workspace context reset, pending-mutation shutdown sync-save skip behavior, duplicate runner ID and progress-delivery failure rejection, additive/destructive sandbox-setting plus theme/prerelease rollback on failed persistence, overlapping modal replacement cancellation safety, collection-import save rollback, startup-smoke/pre-window failure logs, active-element ARIA metadata redaction in structural DOM-state artifacts, source UI, source/packaged sandbox-validation, and packaged-smoke artifact/thrown-error redaction through the shared diagnostics redactor for private keys, request/response aliases, JSON-escaped slash URLs/file URLs, auth schemes, JSON-escaped/double-escaped/nested-JSON camelCase auth-header aliases, and OAuth code/verifier/assertion fields, byte-bounded smoke child output, byte-bounded script-worker stdio lines, timeout-bounded release artifact inspectors and validation children, and CI screenshot/redacted structural DOM/log artifacts for UI smoke failures.
- Diagnostics/privacy matrix and validation, including local-only bounded JSONL diagnostics, user-selected local bundle export, no telemetry/cloud upload/account requirement, default-deny request/response logging for traffic aliases, methods, status codes/categories, sizes, and metadata, explicit off-by-default current-workspace-scoped request/response logging categories stored in managed workspace `localsettings` with PII warnings and partial-save preservation, import/export omission for local diagnostics settings, redaction for standalone Bearer/Basic/Digest/Hawk/Token/OAuth/NTLM/Negotiate auth tokens, comma/semicolon-delimited compound auth-header values with optional whitespace around equals, camelCase auth-header aliases, cookie/certificate-passphrase/generic-credential/assigned exact token/code/state labels, broad camelCase/snake_case/kebab-case token/secret/password/passwd/passphrase/credential suffix aliases, X-API-key, X-access/auth/authorization-token, CSRF/XSRF-token, JWT-token, secret-key, API-secret, subscription-key, access-key, shared-access-key, account/storage/signing/webhook/license/public key, consumer-key/secret, and OAuth-consumer-key/secret aliases, and assigned and bare whitespace-only snake_case/kebab-case/camelCase OAuth-token labels/unquoted multi-word and repeated-whitespace secret/body-alias/responseText/text/variables/script-console/payload-identifier/protocol-message/JWT/private-key/local-path/extended-UNC/Windows-device-path/file-URL/JSON-escaped-slash-URL/mixed-path/bare-host/custom-URL/OAuth-provider-error/OAuth-callback/fragment-token/structured-query/path-param-secret values, escaped/double-escaped/nested-JSON auth-header values, double-escaped nested array/object aliases, and raw/currentValue/example/schema-default aliases plus source/UI/packaged smoke failure output, source/packaged sandbox validation child output, diagnostic event type/outcome/failure-code metadata including compact/delimiter-free token/code/state labels and one-letter token aliases, IPC/export failure messages, and secret-shaped IPC/export error names and codes, diagnostics export IPC waiting for queued local privacy-setting saves, renderer pending-save/current-workspace controls, and tests for record truncation, log rotation, disabled logging, no network/DNS/socket upload, bundle schema, IPC validation, import/export reset, event emitters, UI behavior, and matrix freshness.
- Performance test/docs support rows in the UX/accessibility, workspace durability, non-Postman compatibility, diagnostics/privacy, and production readiness matrices. These rows keep local Performance requirements visible while the implementation is still deferred.
