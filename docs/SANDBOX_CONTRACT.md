...
Status: Postman-import compatibility contract and parity burn-down plan. The current runtime implements the brokered async subset described here in `src/core/scriptRuntime.js`, `src/core/scriptSandbox.js`, and `src/core/scriptWorker.js`, including the Step 2 Postman global surface, Step 3 SDK facades, Step 4 test/variable/run-order behavior, Step 5 brokered network/cookie/package/visualizer/vault work, Step 6 GraphQL/gRPC protocol hook lifecycle work, Step 7 local mock script work, Step 8 Postman import/export preservation work, Step 9 production-claim gating/adversarial/real-world corpus work, and Step 10 separate OS-sandbox platform tracking, with a Linux `bubblewrap` OS isolation backend plus seccomp cBPF syscall policy layered around script workers when available or required. The product target is for imported Postman scripts to run on the first try wherever PostMeter can provide equivalent behavior without giving scripts direct host filesystem, process, shell, Node, Electron, or renderer access.
...
- Current implementation status: `pm.sendRequest` normalizes URL strings, plain objects, SDK `Request` JSON, header object/array/list-like inputs, raw/urlencoded/form-data-text/GraphQL/inline file body forms, common bearer/basic/API-key/OAuth2/client-certificate auth helpers, manual redirect mode, timeout/cancellation signals, callback/promise completion, response final URL, and shared cookie-jar side effects. Proxy options, unsupported auth schemes such as NTLM/digest/AWS signing, and arbitrary local file/binary bodies fail closed with explicit errors.
...
- Supports escaped `{{value}}`, raw `{{{value}}}`, and a bounded Handlebars-compatible block subset: `{{#each}}`, `{{#if}}`, `{{#unless}}`, `{{#with}}`, `{{else}}`, `{{this}}`, `{{@index}}`, `{{@key}}`, `{{@first}}`, `{{@last}}`, `{{@root}}`, parent lookup with `../`, block params, and object-field lookups.
- Supports bounded custom visualizer helpers, block helpers, partials, decorators as registered metadata, compile options, `Handlebars.compile`, `Handlebars.escapeExpression`, and `Handlebars.SafeString` through the sandbox-provided `Handlebars` facade and `pm.visualizer.set(..., options)` helper/partial registration.
...
- Imports and scans Postman script package references into a workspace-visible review flow so missing or unpinned team/external package references are visible before execution.
...
- Match Postman's dynamic code behavior for `Function` and indirect eval only inside the isolated worker context. The default parity profile now allows sandboxed `Function` string execution while direct `eval` remains guarded; escape regressions must stay in the release gate.
...
- Current implementation status: bundled facades and reviewed exact `@team/package`, `npm:package@version`, and `jsr:package@version` cache entries are supported with integrity and dependency policy. Missing imported package references are detected and shown for manual review, but an online fetch-and-review workflow that can satisfy missing references before first run remains a default-import claim blocker.
...
- Current implementation status: the broker now normalizes string/object/SDK request inputs, common headers/body/auth forms, redirect and timeout options, response final URLs, and cookie-jar side effects without exposing raw network primitives. Advanced auth/proxy/client-certificate parity and user-granted imported file/binary body bindings remain default-import claim blockers. Unsupported transport surfaces fail closed rather than granting scripts raw sockets, arbitrary local files, or TLS policy bypass.
...
- Current implementation status: current-request helpers and jar helpers support callback and thenable forms, URL/hostname normalization, Cookie object payloads, expiry/max-age, Secure, HttpOnly storage with script-read hiding, SameSite, Priority, Partitioned, host-only/default-path/prefix validation, duplicate replacement, and staged `pm.sendRequest` jar updates.
...
- Current implementation status: the visualizer facade supports bounded compile options, helpers, block helpers, partials, SafeString, block params, data variables, parent/root lookup, escaping/noEscape, inline scripts, `pm.getData`, sanitized iframe rendering, and integrity-checked reviewed JS/CSS assets injected into the isolated visualizer document. Unreviewed external loads remain blocked by sanitizer and iframe CSP.
...
- Generated docs parity matrix with zero unsupported rows for the supported import surface. `npm run postman:parity:claim` enforces this for default Postman import mode and currently fails closed until `cookies.httponly-parity`, `sendRequest.advanced-auth-proxy`, `sendRequest.file-binary-bindings`, `require.pm.online-fetch-review`, and `grpc.live-desktop-transport` are complete.
...
- Differential harness that runs the same fixtures in Postman Desktop where automatable, latest Newman where supported, and PostMeter, then compares observable output. The first checked-in HTTP-core differential fixture runs through `npm run postman:parity:diff`, with optional live Newman comparison through `npm run postman:parity:diff -- --newman --download-newman`.
...
- `npm run postman:parity:claim` must pass before any full 1:1 Postman script compatibility claim. It is intentionally separate from normal validation so development can continue while claim blockers remain visible.
- `npm run postman:parity:diff` must pass for the local HTTP-core differential fixture; `npm run postman:parity:diff -- --newman --download-newman` is the optional live comparison against the targeted `newman@6.2.2` release when network access is available.
