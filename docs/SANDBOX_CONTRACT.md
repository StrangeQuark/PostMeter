...
- Bundled allowlisted package loading plus reviewed cached team/external package bundles, including scoped npm and exact-version npm/JSR import forms. Unversioned latest npm/JSR imports are intentionally rejected so reviewed code remains reproducible.
...
- Supports reviewed package-cache entries for team-style package specifiers (`@team/package`) and external registry specifiers with exact versions (`npm:package@version`, `npm:@scope/package@version`, `jsr:@scope/package@version`). Postman latest-version forms (`npm:package`, `npm:@scope/package`, `jsr:@scope/package`) are intentionally rejected before review because mutable latest resolution would break reviewed-code reproducibility.
...
| `pm.require` | Support team Package Library specifiers, exact external `npm:`/`jsr:` specifiers, package cache/review/fetch workflow, CommonJS behavior, native ESM behavior for JSR packages, and full bundled library behavior without runtime registry access. Unversioned latest-form external specifiers are a documented security gap from Postman behavior. |
...
- Package fixtures for every built-in library facade, every Postman-listed NodeJS module facade, reviewed team packages, exact external npm packages, exact external JSR packages including native ESM syntax, rejected latest-form external specifiers, missing packages, circular dependencies, and dependency policy failures.
