...
Status: Postman-import compatibility contract and claim-gated implementation record. The current runtime implements the brokered async subset described here in `src/core/sandbox/scriptRuntime.js`, `src/core/sandbox/scriptSandbox.js`, and `src/core/sandbox/scriptWorker.js`, including the Postman global surface, SDK facades, test/variable/run-order behavior, brokered network/cookie/package/visualizer/vault work, metadata-only desktop vault prompts, GraphQL/gRPC protocol hook lifecycle work, parent-side HTTP/gRPC PEM and PFX/P12 mTLS handling, local mock script work, Postman import/export preservation work, production-claim gating/adversarial/real-world corpus work, response/protocol assertion parity, advanced auth parity, JSR ESM package execution, and separate OS-sandbox platform tracking, with Linux `bubblewrap` plus seccomp, Windows AppContainer helper, and macOS seatbelt OS isolation layered around script workers when available or required. The product target is for imported Postman scripts to run on the first try wherever PostMeter can provide equivalent behavior without giving scripts direct host filesystem, process, shell, Node, Electron, or renderer access.
...
- Supports reviewed package-cache entries for team-style package specifiers (`@team/package`) and external registry specifiers with either exact versions (`npm:package@version`, `npm:@scope/package@version`, `jsr:@scope/package@version`) or Postman latest-version form (`npm:package`, `npm:@scope/package`, `jsr:@scope/package`). Latest-version imports must be resolved by the parent fetch/review workflow to a concrete `packageVersion` before script execution, including constrained ESM syntax for reviewed JSR-style modules.
...
- Provides a parent-side fetch-and-review workflow for exact public npm packages, latest public npm packages, exact public JSR packages, latest public JSR packages, and team Package Library-style packages whose source is provided through a reviewed HTTPS source URL. The fetch workflow runs through Electron IPC in the parent process, enforces HTTPS, registry/source host policy, redirect limits, size limits, npm latest dist-tag resolution, JSR latest metadata resolution, npm tarball entrypoint selection, JSR checksum verification, and SHA-256 cache integrity before any package can be saved.
...
Full Postman script import compatibility for the supported default import profile is complete when every item in this section is implemented, documented, covered by golden/differential fixtures, and `npm run postman:parity:claim` reports zero default-import blockers. The current generated matrix is claim-green for the tracked default import profile with row-specific Desktop evidence metadata for behavior-sensitive Desktop rows. This section records both the completed profile and the evidence that must stay current as the sandbox evolves.

Current audited versions:

- Postman Desktop: 11.71.7.
- Postman sandbox package: `postman-sandbox@6.2.2`.
- Postman Desktop runtime package: `postman-runtime@7.50.0`.
- Newman: `newman@6.2.2`, currently the targeted npm latest release.
- Newman runtime package: `postman-runtime@7.39.1`.

Claim gate:

- The default import profile must prefer Postman-compatible behavior over intentionally narrow behavior. Stricter local Settings choices may disable brokered APIs, but the default path for an imported Postman collection should run without manual policy changes.
...
- Maintain a generated parity inventory from the official Postman Sandbox API reference pages and related scripting/runtime docs: overview, variables, dynamic variables, vault, cookies, request, response, sendRequest, authorization helpers, visualizer, test/expect, test examples, require/globals/modules, execution, message, info, mock, state, GraphQL client/interface scripting, gRPC scripting/interface behavior, and audited no-script protocol surfaces.
- The generated inventory source lives in `src/core/diagnostics-release/postmanParityMatrix.js`; the committed machine-readable output is `docs/postman-sandbox-parity-matrix.json`. Regenerate it with `npm run postman:parity:write`, validate structure/staleness with `npm run postman:parity:validate`, and validate the full 1:1 compatibility claim with `npm run postman:parity:claim`.
- The official-docs token audit source lives in `src/core/diagnostics-release/postmanDocsCoverageAudit.js`; the committed output is `docs/postman-docs-coverage-audit.json`. Regenerate it from live official Postman/Newman sources with `npm run postman:docs:write`, validate the committed token-to-row mapping with `npm run postman:docs:validate`, and run `npm run postman:docs:live` before preserving a 1:1 compatibility claim after upstream docs or Newman releases may have changed.
...
- Rows that require Postman Desktop observation must have a completed observation artifact, automated desktop-runner result, or installed Desktop runtime/source audit artifact before they can be marked complete. The checked-in desktop observation template is not evidence by itself, and the parity validator rejects implemented desktop-required rows that rely only on the template. Behavior-sensitive rows must declare `desktopEvidence: "row-specific-behavior"` in the generated matrix and must be backed by explicit `rowEvidence` metadata for that exact row; a broad installed runtime/source audit can prove package versions and exported hook presence, but it is not sufficient by itself for exact behavior claims.
...
- Implement CommonJS semantics expected by Postman packages: module caching, `module.exports`, `exports`, dependency resolution within reviewed package bundles, default export interop patterns used by npm packages and CommonJS-compatible reviewed packages, circular dependency behavior, and error behavior.
- Implement native ESM execution semantics for reviewed JSR packages: `import`/`export` syntax, default and named exports, dependency graph loading, module cache identity, and ESM parse/runtime error behavior inside the isolated VM without granting filesystem, registry, or host Node access.
...
- Current implementation status: PostMeter vendors a generated `postman-sandbox@6.2.2` browser bootcode bundle at `src/core/sandbox/postmanSandboxBootcodeBundle.js`, verifies the inflated artifact against a checked-in SHA-256 digest, executes it inside the script VM with Browserify entry modules disabled, captures its internal package resolver, restores PostMeter's allowlisted `require`, and exposes only normalized bundled package names through the existing package policy. Ajv, Chai, Cheerio, CryptoJS, csv-parse/lib/sync, Lodash, Moment, Postman Collection SDK, UUID, XML2JS, and legacy JSON/TV4/Backbone package surfaces now come from Postman's pinned bundle. `stream` and `timers` remain PostMeter script-safe facades with brokered timer behavior because the raw Browserify shims assume ambient window/timer behavior. Reviewed `@team/package`, exact and scoped npm package entries, exact and scoped JSR package entries, and Postman latest-version `npm:`/`jsr:` entries support SHA-256 integrity, normalized multi-file bundles, package metadata entrypoints, package.json directory entrypoints, local relative modules, `.js`/`.cjs`/`.mjs`/`.json` resolution, constrained ESM import/export transpilation, reviewed dependency aliases and subpaths, module caching, circular dependencies, `module.exports`/`exports`, `module.require`, `require.resolve`, `__filename`, `__dirname`, default and named export object patterns, dependency caps, source/file/package caps, duplicate/missing dependency denial, and deterministic cannot-resolve errors. Missing imported package references are detected, shown for review, and can be resolved through parent-side npm/JSR exact-version or latest-version fetches or reviewed HTTPS team-package source URLs before first run.
...
- Support common SDK methods and semantics used by imported scripts: `get`, `has`, `all`, `idx`, `each`, `map`, `filter`, `add`, `append`, `prepend`, `insert`, `insertAfter`, `upsert`, `remove`, `clear`, `populate`, `repopulate`, `one`, `indexOf`, `count`, `find`, `reduce`, `toObject`, `toJSON`, `toString`, `clone`, `valueOf`, disabled-item filtering, case-insensitive headers, multi-value headers, URL path/query mutation, and safe object identity behavior.
...
- Current implementation status: `require('postman-collection')` now resolves to Postman's bundled Collection SDK from the pinned VM bundle. Live `pm.request` and `pm.response` use hardened PostMeter SDK-style facades because they are transaction-aware views over PostMeter request/response state. Those facades cover SDK-style list operations, object-predicate `filter(...)` semantics, URL/query/header/body/auth mutation, `clone`/`toJSON`/`toString`, disabled-item filtering, case-insensitive and multi-value headers, response cookies from `Set-Cookie`, metadata/trailers/messages, protocol request metadata/messages, `pm.message`, and adversarial constructor/prototype escape tests. Direct constructors remain masked as part of the host-escape boundary. File/binary body resolution remains governed by the network parity section; scripts still must not gain arbitrary filesystem reads.
...
- Implement Postman response assertion helpers: `pm.response.to.have.status`, status-name strings such as `"Created"`, `responseTime`, `header`, `body`, `jsonBody`, `jsonSchema`, protocol metadata/trailer/message helpers, message-list assertion helpers, status-family helpers, `pm.response.to.not.be.error`, `pm.response.to.be.error` if supported, and the chain behavior around `to`, `be`, `have`, and `not`.
...
- Current implementation status: PostMeter supports deterministic `pm.test` registration order, `pm.test.skip`, `pm.test.index`, duplicate names, skipped-result metadata, sync/callback/promise/mixed callback behavior, nested async tests, a broad Chai-compatible `pm.expect`/`chai.assert` facade, arbitrary-value JSON-schema assertions, response status-name strings, response-time assertions, response status/header/body/jsonBody/jsonSchema helpers, response error/non-error helpers, protocol metadata/trailer/message assertion helpers, and protocol message-list assertion helpers. Exact Chai failure text is matched only where practical and remains a differential-observation refinement item, not a sandbox-boundary exception.
...
- Match `pm.execution.setNextRequest` by name, ID, and `null`; pre-request-only `pm.execution.skipRequest` that stops remaining pre-request code and prevents post-response tests from running; and collection-run branching/loop semantics, including how failures, stops, and skipped requests affect later phases.
...
- Current implementation status: HTTP pre-request and post-response scripts run in imported collection/folder/request hierarchy order, GraphQL Before query/After response and gRPC Before invoke/On message/After response hooks run through the same isolated worker lifecycle, `pm.info` reports `prerequest`/`test`/`beforeQuery`/`beforeInvoke`/`onIncomingMessage`/`afterResponse` plus iteration and request metadata, `pm.execution.location` is an array path with string `.current` and preserved request metadata, `setNextRequest` and pre-request `skipRequest` control collection order, post-response scripts do not expose `skipRequest`, and `runRequest` is brokered with request ID/name/link resolution, referenced scripts, variable overrides, skip-to-null behavior, caller-visible referenced tests, collection/depth limits, and the Postman 10-call per-script limit.
...
- Current implementation status: the broker normalizes string/object/SDK request inputs, common headers/body/auth forms, Digest/Hawk/AWS/OAuth1/NTLM/Akamai EdgeGrid/JWT Bearer/ASAP auth forms, proxy options, redirect and timeout options, configured client-certificate bindings, user-granted imported file/binary/form-data body bindings, response final URLs, and cookie-jar side effects without exposing raw network primitives. Imported file references are bound in managed workspace `localsettings` sandbox settings and read only by the parent sender as approved regular files under the request body cap; Newman live differential coverage exercises imported file/body transport, while focused sandbox tests cover `pm.sendRequest` binding and unbound-path denial because Newman disallows script-originated file uploads. Primary pre-request auth mutations may select configured client-certificate bindings by `certificateId`, but script-injected direct certificate, key, PFX/P12, CA, and passphrase fields are ignored before the parent HTTP sender can read local certificate paths. Unsupported transport surfaces fail closed rather than granting scripts raw sockets, arbitrary local files, or TLS policy bypass.
...
- Current implementation status: current-request helpers and jar helpers match the observed `newman@6.2.2` default-import profile for `HttpOnly` script readability. `pm.cookies.get`/`has`/`toObject` expose in-scope `HttpOnly` values synchronously, `pm.cookies.jar().getAll` returns a CookieList facade, jar set/replacement/unset/clear operations affect `HttpOnly` cookies, and `pm.sendRequest` response-cookie jar updates preserve readable `HttpOnly` values. PostMeter also supports current-request `pm.cookies.set/unset` as compatibility extensions, but those methods are tracked out of scope for the official Postman default-import claim because the current Postman current-request cookie reference only documents `has`, `get`, and `toObject`. The installed Postman Desktop 11.71.7 runtime/source audit records matching packaged cookie storage behavior through `postman-runtime` 7.50.0, `postman-collection` 5.2.0, and `@postman/tough-cookie` 4.1.3-postman.1. Script cookie access remains gated by local Settings.
...
- Current implementation status: GraphQL imports preserve body mode/query/variables/operation name and execute Before query/After response around the brokered HTTP sender, including subscription-style response message normalization into `pm.response.messages`. gRPC imports preserve method path, proto/service/method metadata, metadata, outgoing messages, method type, response metadata/trailers/status/messages, and execute Before invoke/On message/After response through a parent-owned `@grpc/grpc-js` transport for unary, client-streaming, server-streaming, and bidirectional-streaming methods. Proto source/path, include dirs, TLS material, client options, and client-certificate auth are snapshotted before Before invoke and remain parent-owned; scripts can mutate metadata, outgoing messages, method path, and target URL through the Postman-compatible request facade, but never receive raw sockets, channels, call handles, filesystem, or TLS/certificate handles. HTTP and gRPC mTLS support parent-side PEM cert/key material, encrypted PEM key passphrases where the transport needs decrypted PEM, regular-file and byte-capped certificate reads, and shared `node-forge` PFX/P12 extraction into in-memory PEM buffers. Primary HTTP/GraphQL request auth mutations and `pm.sendRequest` may select only configured client-certificate bindings by `certificateId`; script-injected direct certificate, key, PFX/P12, CA, and passphrase fields are ignored or rejected before the parent transport reads certificate files. When a request uses `certificateId`, gRPC resolves that ID through the frozen transport environment, uses only the configured certificate binding material, and fails closed if the binding is unavailable instead of falling back to direct request paths. Coverage includes no decrypted temp PEM files, wrong-passphrase, encrypted-key failure, missing-file, missing-binding, non-file/oversized-bundle denial, missing-private-key, malformed-bundle, CA-mismatch, primary HTTP auth-mutation denial, and script-mutation-denial cases for the gRPC protocol path, live PEM/PFX HTTP mTLS coverage for request execution, and live PEM/PFX gRPC mTLS coverage across unary, client-streaming, server-streaming, and bidirectional calls. Protocol metadata/trailer/message assertion helpers, request/response message-list assertion helpers, and hardened `pm.message.timestamp` Date behavior are implemented. The generated parity matrix records the current official-docs WebSocket/Socket.IO audit as an implemented no-script surface because Postman does not document saved/importable WebSocket script hooks.
...
- Current implementation status: PostMeter has a core loopback local mock runner in `src/core/runtime/localMockServer.js`. Imported `mock` event scripts are preserved, package references in mock scripts are detected, request and saved-example IDs are retained when exported by Postman, and the runner supports method/path-only matching, `:path` and `{{path}}` variables, saved-example fallback, top-level mock-script `await`/`return`, `pm.mock.matchRequest`, `pm.mock.sendExample`, Express-style `req`/`res` helpers including `req.json()`, response status/header/body helpers, and `pm.state` through parent-brokered operations with per-script operation limits, per-value/session size caps, per-session storage, reset/clear controls, and commit/rollback behavior tied to script phase success. `pm.mock`, `pm.state`, `req`, and `res` are only injected when `context.mock.enabled` is present.
...
- Generated docs parity matrix with explicit unsupported rows for every known supported-import gap. `npm run postman:parity:claim` enforces this for default Postman import mode and currently passes with zero default-import blockers.
- The current machine-readable parity inventory is committed at `docs/postman-sandbox-parity-matrix.json` and generated by `src/core/diagnostics-release/postmanParityMatrix.js`; stale or malformed output fails `npm run postman:parity:validate`.
- The current official-docs coverage audit is committed at `docs/postman-docs-coverage-audit.json` and generated by `src/core/diagnostics-release/postmanDocsCoverageAudit.js`; unmapped official Postman script, SDK, package-import, or Newman latest-version tokens fail `npm run postman:docs:validate`, and `npm run postman:docs:live` must pass before repeating the full compatibility claim against current upstream docs.
...
| `pm.expect` and Chai | Match Postman's Chai-compatible BDD behavior, bundled `chai` package behavior, arbitrary-value JSON schema validation, response assertion chains, protocol message assertion chains, and practical error messages. |
...
| `pm.request` | Expose full Postman Collection SDK-compatible request objects for HTTP, GraphQL, and gRPC, including URL, headers, query, body, auth, metadata, messages, method path, mutation rules, protocol assertion helpers, and `toJSON`/`toString`/list APIs. |
| `pm.response` | Expose full Postman response objects, including `text`, `json`, code, status text, headers, cookies where exposed, response time, response size, metadata, trailers, messages, binary/streaming handling, status-name/response-time helpers, and response/protocol assertion helpers. |
| `pm.message` | Expose incoming streaming message data and timestamp for gRPC On message scripts, including Postman's documented `Date` timestamp behavior. |
...
| `pm.require` | Support team Package Library specifiers, exact and latest-form external `npm:`/`jsr:` specifiers, package cache/review/fetch workflow, CommonJS behavior, native ESM behavior for JSR packages, and full bundled library behavior without runtime registry access. |
...
| GraphQL scripts | Support Before query and After response hooks, GraphQL client interface behavior, subscription/multiple-response message access, package imports, variable interpolation, auth/headers, request/response access, and run-order semantics. |
| gRPC scripts | Support Before invoke, On message, and After response hooks, streaming lifecycles, metadata, trailers, messages, message-list assertions, cancellation, package imports, and `pm.message`. |
...
- Package fixtures for every built-in library facade, every Postman-listed NodeJS module facade, reviewed team packages, exact and latest-form external npm packages, exact and latest-form external JSR packages including native ESM syntax, missing packages, circular dependencies, and dependency policy failures.
...
- Golden fixtures from Postman/Newman-style runs where practical, plus checked-in raw and normalized `newman@6.2.2` JSON reports for approved Newman-compatible differential suites.
...
- `npm run production:readiness:validate` must pass, proving `docs/production-readiness-matrix.json` is current with `src/core/diagnostics-release/productionReadinessMatrix.js`. `npm run production:readiness:claim` is the stronger stable-release gate and intentionally fails until every release-blocking row is validated. `npm run production:readiness:claim:beta` and `npm run production:readiness:claim:rc` expose lower beta and release-candidate thresholds; RC may accept explicit documented matrix waivers, while stable does not.
- Packaged Linux, Windows, and macOS assertions must prove the OS sandbox backend still works after packaging, including packaged path/ASAR behavior and script-worker execution through the platform backend.

The sandbox may be called production-ready only when the implementation, docs, compatibility matrix, official-docs coverage audit, and verification suite all match this contract. Full Postman script compatibility may be claimed only when `docs/COMPATIBILITY.md` no longer lists unsupported Postman script APIs for the supported import surface, `npm run postman:parity:claim` proves the generated parity matrix has zero unsupported rows for default Postman import mode, and the docs coverage gates prove current official Postman/Newman tokens are mapped.
