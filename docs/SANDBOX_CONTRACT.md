...
- Native Windows and macOS syscall-policy backends require separate platform implementations before PostMeter can claim equivalent OS-level coverage on those platforms. Native helper binaries and native Node addons are acceptable for those AppContainer/seatbelt-style launchers when they are owned by the release process and covered by packaged validation. Current code has a fail-closed Windows helper launch contract and a macOS `sandbox-exec`/seatbelt launcher path where available, but the platform claim stays blocked until native runner source and packaged probes prove the required behavior.
...
- `npm run sandbox:platform:claim` must pass before any platform-equivalent full OS sandbox claim. It is intentionally separate from `npm run postman:parity:claim` and currently fails closed until native Windows/macOS backend validation rows are complete. Linux is no longer blocked on a deny-by-default seccomp decision because the maintainer accepted the current `bubblewrap` plus dangerous-syscall seccomp policy for the current Linux claim.
