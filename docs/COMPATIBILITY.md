...
- Platform OS sandbox completion is tracked separately from Postman API parity in `docs/os-sandbox-platform-matrix.json`, generated from `src/core/osSandboxPlatformMatrix.js`. `npm run sandbox:platform:validate` keeps the matrix current; `npm run sandbox:platform:claim` must pass before PostMeter claims equivalent full OS sandbox coverage across Linux, Windows, and macOS. The current code has accepted Linux coverage, a fail-closed Windows helper contract, and a macOS seatbelt launcher path, but Windows/macOS platform-equivalent claims remain blocked until native-runner packaged probes prove the OS boundary.
...
- Keep `npm run postman:parity:claim`, `npm run postman:docs:validate`, and a current `npm run postman:docs:live` sweep green before preserving the tracked Postman script import claim; future Postman/Newman changes should be added as explicit matrix rows. Native Windows/macOS OS sandbox backends are separate platform-security work tracked by `npm run sandbox:platform:claim` and are not part of the Postman API parity claim.
- Native Windows and macOS syscall-policy backends are not implemented yet; those platforms currently rely on the child process, Node permission flags, hardened `vm` runtime, and broker boundary until native-helper or native-addon backed platform launchers are added and validated. Linux's current `bubblewrap` plus dangerous-syscall seccomp policy is accepted for the current Linux claim; a maintained deny-by-default seccomp-BPF allowlist is optional future hardening.
