...
If a workspace uses a newer schema version that this app does not support, PostMeter refuses to load it. It does not quarantine or overwrite that file, because a newer PostMeter build may still be able to read it.
...
Reviewed script packages are stored as workspace-local metadata in the current managed workspace's `localsettings` sandbox settings. If an imported script reports that a package is missing, unreviewed, duplicated, or has an integrity mismatch, open Settings and review the package cache status.
...
If a script keeps prompting, either choose a broader grant for the intended trust boundary or open Settings and review the `Script vault access` and vault grant state. Imported collections cannot silently grant vault access by themselves.
