...
Reviewed script packages are stored as workspace-local metadata in `settings.json` under the current workspace's sandbox settings. If an imported script reports that a package is missing, unreviewed, duplicated, or has an integrity mismatch, open Settings and review the package cache status.
