...
Keep DOM IDs stable unless tests and IPC-facing workflows are migrated at the same time. The behavior-preserving renderer split is complete enough that new work should usually extend the existing helper modules instead of growing `renderer.js` again.
...
IPC channel names and preload APIs are compatibility contracts. Refactors must preserve them unless a migration is explicitly underway. The source-owned Electron security matrix enumerates every channel, and the matrix validator plus focused tests fail if source channels and documented channel rows diverge.
...
- `docs/SANDBOX_CONTRACT.md` is the source of truth for script sandbox compatibility, security boundaries, broker behavior, side-effect transactions, and Performance scripting scope. The current claim-gated Postman script parity target is Postman Desktop 11.71.7 with `postman-sandbox@6.2.2` and Postman Runtime 7.50.0, plus Newman 6.2.2 with Postman Runtime 7.39.1 for Newman-compatible surfaces.
...
Each runner stores `{ id, name, environmentId, allowEnvironmentMutation, stopOnFailure, capturePolicy, requests }`. Runner requests are independent request objects with local IDs. Importing an individual collection request or an entire collection deep-clones those requests into the runner, so runner edits and request-local script mutations do not change the source collection request.
...
Performance execution is owned by core/main-process modules. The engine runs local bounded iterations through the existing request/script lifecycle, streams progress, supports cancellation, aggregates latency/status/error summaries, rejects unsafe safety-cap combinations, and writes request/sample rows into the single reusable temp result store instead of returning unbounded arrays through IPC. High-volume Performance runs aggregate exact core metrics with streaming counters instead of retaining per-request summary objects in the main-process heap, route primary requests through an explicit keep-alive Node HTTP(S) agent capped to the test concurrency, enforce a bounded per-request timeout, and coalesce fast progress events before they cross Electron IPC. Each V1 type has a scoped input contract: Full Endpoint Diagnosis is a one-click staged local report with Quick, Medium, and Extended scopes that choose the diagnostic request budget, raise the duration cap for deeper runs, capture transport timing, endpoint behavior, passive security/configuration signals, local-client confidence, and UAT CSV diagnostics; latency is single-user samples; throughput is fixed requests at configured concurrency; concurrency multiplies requests per virtual user; stress and ramp execute stepped stages from start users to peak users and reject descending start/peak plans; spike validates baseline users multiplied by the spike multiplier; soak runs against duration and request caps without exposing irrelevant iterations. The Performance sidebar calibration flow starts a temporary `127.0.0.1` HTTP server, runs a short warmup, walks a deterministic target-rate probe ladder, stops after repeated probe failures, and verifies only a small target band around the observed edge. The default calibration budget is capped to about two minutes, with a hard runtime guard, so it cannot expand into a long near-edge search. The reported sustained local RPS is the highest contiguous confirmed target, so an isolated high pass cannot override a failed lower neighbor or a target where local p95 latency jumps enough to show PostMeter is queueing work. The modal streams calibration progress with the current phase, target, pass, request count, and progress bar. Calibration also reports a conservative planning cap separately from the measured maximum, and judges target hit rate, request completion, scheduler lag, event-loop delay, errors, repeatability, and stability so it does not treat a short burst as a sustained local limit. Dedicated `performance:*` IPC/preload channels are least-privilege, schema-validated, sender-validated, and listed in the Electron security matrix.

## Runtime Result Store

Runner and Performance execution share one app-managed temp SQLite file at `userData/runtime/postmeter-current-results.sqlite`. Starting a new result-producing run closes and replaces the previous temp store, and app shutdown closes and deletes the current SQLite file plus journal/WAL/SHM sidecars, so PostMeter does not keep historical run files or pile up background exports between sessions. The store uses DELETE journaling to avoid persistent WAL sidecars, keeps core request/sample metrics for every row, stores optional detail fields according to the saved capture policy, and exposes bounded page/detail IPC methods for the renderer. Export commands stream CSV or JSON from the current temp store only when the user chooses an export path.

Capture policies are intentionally small and match the current result panes: response body preview mode and size, pre-request output, post-request output, script logs, local variables, plus Performance-only headers and transport timings. Before a run starts, runtime IPC estimates the SQLite file size from the planned request count, normalized capture policy, request metadata size, and row/index overhead, then returns effective available disk space including the current temp DB that will be replaced. The renderer warns when the estimate is within 1 GB of effective free space and disables Continue when the estimate exceeds effective free space. Runs above high-volume thresholds keep core metrics but force expensive detail capture down to failed/sampled previews, disable per-request logs/variables at 100k+, and disable script output, response headers, and transport timings at 500k+ outside Full Endpoint Diagnosis so 100k-1m request runs remain usable.
