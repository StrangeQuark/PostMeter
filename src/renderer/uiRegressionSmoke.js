(function attachUiRegressionSmoke(global) {
  ...
  async function assertSidebarPanelSmoke() {
    ...
    try {
      ...
      assertUiSmoke(performanceCsvMenuLabels.join('|') === 'Turn On|Edit', `Performance CSV menu should expose toggle and edit actions. labels=${performanceCsvMenuLabels.join('|')}`);
      ...
      assertUiSmoke(performanceCsvMenuLabels.join('|') === 'Turn Off|Edit', `Performance CSV menu should update the toggle label when enabled. labels=${performanceCsvMenuLabels.join('|')}`);
      $('performanceToggleCsvVariablesButton').click();
      ...
      dispatchChange($('performanceAuthTypeSelect'));
      $('performanceAuthBearerTokenInput').value = '{{ACCESS_TOKEN}}';
      ...
      assertUiSmoke(performanceAutoRefreshAccessOption.textContent === 'Use Refreshing Access Token', 'Performance request refreshing access token option should use the user-facing label.');
      assertUiSmoke(
        !performanceAutoRefreshAccessOption.hidden && !performanceAutoRefreshAccessOption.disabled,
        `Performance request refreshing access token option should be selectable. hidden=${performanceAutoRefreshAccessOption.hidden} disabled=${performanceAutoRefreshAccessOption.disabled} value=${$('performanceAuthTypeSelect').value} requestAuth=${performanceTest.request.auth?.type || ''} refreshEnabled=${performanceTest.authRefresh?.enabled === true} refreshType=${performanceTest.authRefresh?.authType || ''}`
      );
      ...
      assertUiSmoke(runnerCsvMenuLabels.join('|') === 'Turn On|Edit', `Runner CSV menu should expose toggle and edit actions. labels=${runnerCsvMenuLabels.join('|')}`);
      ...
      assertUiSmoke(runnerCsvMenuLabels.join('|') === 'Turn Off|Edit', `Runner CSV menu should update the toggle label when enabled. labels=${runnerCsvMenuLabels.join('|')}`);
      $('runnerToggleCsvVariablesButton').click();
      ...
      $('authTypeSelect').value = 'bearer';
      dispatchChange($('authTypeSelect'));
      $('authBearerTokenInput').value = '{{ACCESS_TOKEN}}';
      dispatchInput($('authBearerTokenInput'));
      collectRequestFromEditor();
      ...
      assertUiSmoke(
        ...
        'Turning runner refreshing auth on should not auto-select refreshing auth for runner requests.'
      );
      runnerLocalRow = $('runnerRequestList').querySelector('.runner-request-row');
      const runnerRefreshAuthCheckbox = runnerLocalRow?.querySelector('.runner-row-refresh-auth input[type="checkbox"]');
      assertUiSmoke(runnerRefreshAuthCheckbox, 'Runner request rows should show a refreshing auth checkbox when runner refreshing auth is enabled.');
      assertUiSmoke(runnerRefreshAuthCheckbox.checked === false, 'Runner refreshing auth checkbox should default off for each request.');
      runnerRefreshAuthCheckbox.checked = true;
      dispatchChange(runnerRefreshAuthCheckbox);
      await nextPaint();
      assertUiSmoke(runner.requests[0].auth?.type === 'autoRefresh', 'Checking a runner request refreshing auth checkbox should set the request auth type.');
      assertUiSmoke(runner.requests[0].refreshingAuthOriginalAuth?.type === 'bearer', 'Checking a runner request refreshing auth checkbox should preserve the previous auth type.');
      assertUiSmoke(runner.requests[0].refreshingAuthOriginalAuth?.token === '{{ACCESS_TOKEN}}', 'Checking a runner request refreshing auth checkbox should preserve the previous auth details.');
      editRunnerRequest(runner, runnerLocalRequest);
      await nextPaint();
      const runnerAutoRefreshAccessOption = $('authTypeSelect').querySelector('option[value="autoRefresh"]');
      assertUiSmoke(runnerAutoRefreshAccessOption, 'Runner request Auth Type list should include Use Refreshing Access Token when refreshing auth is active.');
      assertUiSmoke(runnerAutoRefreshAccessOption.textContent === 'Use Refreshing Access Token', 'Runner request refreshing access token option should use the user-facing label.');
      assertUiSmoke(!runnerAutoRefreshAccessOption.hidden && !runnerAutoRefreshAccessOption.disabled, 'Runner request refreshing access token option should be selectable.');
      assertUiSmoke($('authTypeSelect').value === 'autoRefresh', 'Runner request Auth Type should show Use Refreshing Access Token after checkbox selection.');
      assertUiSmoke($('authTypeSelect').disabled === true, 'Runner request Auth Type select should be locked while the row uses refreshing auth.');
      selectRunnerItem(runner.id);
      runnerLocalRow = $('runnerRequestList').querySelector('.runner-request-row');
      const runnerRefreshAuthRestoreCheckbox = runnerLocalRow?.querySelector('.runner-row-refresh-auth input[type="checkbox"]');
      assertUiSmoke(runnerRefreshAuthRestoreCheckbox?.checked === true, 'Runner refreshing auth checkbox should reflect requests already using refreshing auth.');
      runnerRefreshAuthRestoreCheckbox.checked = false;
      dispatchChange(runnerRefreshAuthRestoreCheckbox);
      await nextPaint();
      assertUiSmoke(runner.requests[0].auth?.type === 'bearer', 'Unchecking a runner request refreshing auth checkbox should restore the previous auth type.');
      assertUiSmoke(runner.requests[0].auth?.token === '{{ACCESS_TOKEN}}', 'Unchecking a runner request refreshing auth checkbox should restore the previous auth details.');
      assertUiSmoke(!runner.requests[0].refreshingAuthOriginalAuth, 'Unchecking a runner request refreshing auth checkbox should clear the saved previous auth.');
      editRunnerRequest(runner, runnerLocalRequest);
      await nextPaint();
      assertUiSmoke($('authTypeSelect').disabled === false, 'Runner request Auth Type select should unlock after turning off the row refreshing auth checkbox.');
      $('authTypeSelect').value = 'apiKey';
      dispatchChange($('authTypeSelect'));
      $('authApiKeyLocationSelect').value = 'header';
      dispatchChange($('authApiKeyLocationSelect'));
      $('authApiKeyNameInput').value = 'X-API-Key';
      dispatchInput($('authApiKeyNameInput'));
      $('authApiKeyValueInput').value = 'stale-api-key';
      dispatchInput($('authApiKeyValueInput'));
      collectRequestFromEditor();
      selectRunnerItem(runner.id);
      $('runnerAuthRefreshTypeSelect').value = 'apiKey';
      dispatchChange($('runnerAuthRefreshTypeSelect'));
      $('runnerAuthRefreshApiKeyLocationSelect').value = 'query';
      dispatchChange($('runnerAuthRefreshApiKeyLocationSelect'));
      $('runnerAuthRefreshApiKeyNameInput').value = 'api_key';
      dispatchInput($('runnerAuthRefreshApiKeyNameInput'));
      runnerLocalRow = $('runnerRequestList').querySelector('.runner-request-row');
      const runnerApiKeyRefreshCheckbox = runnerLocalRow?.querySelector('.runner-row-refresh-auth input[type="checkbox"]');
      assertUiSmoke(runnerApiKeyRefreshCheckbox, 'API key runner refreshing auth should show the row checkbox.');
      assertUiSmoke(
        runnerLocalRow?.querySelector('.runner-row-refresh-auth span')?.textContent === 'Use refreshing API key',
        'API key runner refreshing auth checkbox should use the API-key-specific label.'
      );
      runnerApiKeyRefreshCheckbox.checked = true;
      dispatchChange(runnerApiKeyRefreshCheckbox);
      await nextPaint();
      assertUiSmoke(runner.authRefresh.apiKeyLocation === 'query' && runner.authRefresh.apiKeyName === 'api_key', 'API key refreshing auth should save the target location and name.');
      assertUiSmoke(runner.requests[0].auth?.type === 'autoRefresh', 'Checking an API key runner request refreshing auth checkbox should set the request auth type.');
      editRunnerRequest(runner, runnerLocalRequest);
      await nextPaint();
      ...
      runnerApiKeyRefreshRestoreCheckbox.checked = false;
      dispatchChange(runnerApiKeyRefreshRestoreCheckbox);
      await nextPaint();
      editRunnerRequest(runner, runnerLocalRequest);
      await nextPaint();
      $('authTypeSelect').value = 'bearer';
      ...
      $('urlInput').value = 'https://auth.example.test/refresh-token';
      dispatchInput($('urlInput'));
      collectRequestFromEditor();
      selectRunnerItem(runner.id);
      openNewAuthRefreshRequest('runner', 'access');
      await nextPaint();
      const runnerAutoRefreshRefreshOption = $('authTypeSelect').querySelector('option[value="autoRefreshRefreshToken"]');
      assertUiSmoke(runnerAutoRefreshRefreshOption, 'Auth refresh request Auth Type list should include Refreshing Auth Refresh Token when a refresh-token request is configured.');
      assertUiSmoke(runnerAutoRefreshRefreshOption.textContent === 'Refreshing Auth Refresh Token', 'Auth refresh request refresh-token option should use the user-facing label.');
