...
const ELECTRON_IPC_CHANNELS = Object.freeze([
  ...
  ipcChannel('app:auto-update-status', 'renderer-to-main', 'Automatic update status query returns validated service state without accepting renderer update metadata.', ['electron/appIpc.js', 'electron/preload.js', 'electron/autoUpdateService.js', 'src/core/ipcValidation.js'], ['test/electron/appIpc.test.js', 'test/electron/autoUpdateService.test.js', 'test/electron/ipcValidation.test.js']),
  ipcChannel('app:install-update', 'renderer-to-main', 'Automatic update install request is available only after the main-process update service has downloaded a validated update.', ['electron/appIpc.js', 'electron/preload.js', 'electron/autoUpdateService.js', 'src/core/ipcValidation.js'], ['test/electron/appIpc.test.js', 'test/electron/autoUpdateService.test.js', 'test/electron/ipcValidation.test.js']),
  ...
  ipcChannel('app:set-menu-shortcuts-ignored', 'renderer-to-main', 'Shortcut editor focus can temporarily suppress native menu accelerators so user-assigned shortcuts are captured in the renderer.', ['electron/appIpc.js', 'electron/preload.js', 'electron/mainWindow.js'], ['test/electron/appIpc.test.js', 'test/electron/mainWindowSmoke.test.js', 'test/electron/rendererBootstrap.test.js']),
  ipcChannel('clipboard:writeText', 'renderer-to-main', 'Explicit user copy actions send bounded text to the main-process clipboard API through the trusted renderer IPC wrapper.', ['electron/appIpc.js', 'electron/preload.js', 'src/renderer/renderer.js'], ['test/electron/appIpc.test.js', 'test/electron/rendererBootstrap.test.js']),
  ...
  ipcChannel('updates:status', 'main-to-renderer', 'Automatic update status events are validated in the main process and sanitized again by preload before renderer delivery.', ['electron/main.js', 'electron/preload.js', 'electron/autoUpdateService.js', 'src/core/ipcValidation.js'], ['test/electron/preloadMenuActions.test.js', 'test/electron/autoUpdateService.test.js', 'test/electron/ipcValidation.test.js']),
  ...
  ipcChannel('runner:export', 'renderer-to-main', 'Collection-run export validates result/format payloads before a main-process save picker and streams store-backed CSV/JSON/HTML exports from the current runtime result store.', ['electron/runtimeIpc.js', 'electron/preload.js', 'electron/fileDialogs.js', 'src/core/ipcValidation.js', 'src/core/runtimeResultStore.js', 'src/core/resultHtmlReport.js'], ['test/electron/runtimeIpc.test.js', 'test/electron/ipcValidation.test.js', 'test/electron/runtimeResultStore.test.js', 'test/electron/resultHtmlReport.test.js']),
  ipcChannel('runner:estimateResultStore', 'renderer-to-main', 'Collection-run result-store estimation validates runner/config payloads before returning a rough SQLite size estimate, effective available disk space, and whether the renderer must block Continue.', ['electron/runtimeIpc.js', 'electron/preload.js', 'src/core/ipcValidation.js', 'src/core/runtimeResultStore.js'], ['test/electron/runtimeIpc.test.js', 'test/electron/ipcValidation.test.js', 'test/electron/runtimeResultStore.test.js']),
  ...
  ipcChannel('performance:start', 'renderer-to-main', 'Performance-test start validates ID, saved test payload, environment, capture settings, safety limits, and progress payloads before local execution writes high-volume samples to the reusable local runtime result store.', ['electron/runtimeIpc.js', 'electron/preload.js', 'src/core/ipcValidation.js', 'src/core/performanceRunner.js', 'src/core/runtimeResultStore.js', 'src/core/resultCapturePolicy.js'], ['test/electron/runtimeIpc.test.js', 'test/electron/ipcValidation.test.js', 'test/electron/performanceRunner.test.js', 'test/electron/runtimeResultStore.test.js']),
  ...
  ipcChannel('performance:exportResult', 'renderer-to-main', 'Performance-result export validates bounded run output before writing JSON, CSV, or HTML through a main-process save picker and streams store-backed exports from the current runtime result store.', ['electron/runtimeIpc.js', 'electron/preload.js', 'electron/fileDialogs.js', 'src/core/ipcValidation.js', 'src/core/performanceFormats.js', 'src/core/runtimeResultStore.js', 'src/core/resultHtmlReport.js'], ['test/electron/runtimeIpc.test.js', 'test/electron/ipcValidation.test.js', 'test/electron/performanceRunner.test.js', 'test/electron/runtimeResultStore.test.js', 'test/electron/resultHtmlReport.test.js']),
  ipcChannel('performance:estimateResultStore', 'renderer-to-main', 'Performance result-store estimation validates the saved test payload before returning a rough SQLite size estimate, effective available disk space, and whether the renderer must block Continue.', ['electron/runtimeIpc.js', 'electron/preload.js', 'src/core/ipcValidation.js', 'src/core/runtimeResultStore.js'], ['test/electron/runtimeIpc.test.js', 'test/electron/ipcValidation.test.js', 'test/electron/runtimeResultStore.test.js']),
...
function buildDiagnosticsPrivacyMatrix() {
  return matrix('diagnostics-privacy', 'src/core/productionSupportMatrices.js', [
    ...
    row('privacy.performance-results', 'Performance results', 'Local Performance tests keep configurations, progress, capture-policy-controlled result samples, exports, diagnostics, and failure artifacts bounded; high-volume runs use one reusable local runtime result store with pre-run disk-space warnings, shutdown cleanup, and guardrails for optional response/body/script capture, and no Performance diagnostic path introduces telemetry, cloud upload, account-gated execution, distributed result collection, or retained run history.', 'implemented', {
      evidenceRefs: ['docs/TECH_SPECS.md', 'docs/RELEASE_READINESS.md', 'src/core/performanceRunner.js', 'src/core/resultCapturePolicy.js', 'src/core/runtimeResultStore.js', 'electron/runtimeIpc.js', 'electron/main.js'],
...
function buildUxAccessibilityMatrix() {
  return matrix('ux-accessibility', 'src/core/productionSupportMatrices.js', [
    ...
    row('workflow.collection-runner', 'Collection runner', 'Workspace-owned desktop runners expose empty-pane creation, toolbar creation, collection/request import with multi-select, runner-owned request editing, idle, running, cancellation, capture settings, pre-run result-store size warnings, paged store-backed high-volume results up to one million expanded requests, export, pass/fail, pre-run save failure, dirty tab, environment mutation, row reorder/delete, split result details, and failed-operation states without writing partial result exports or mutating source collection requests.', 'implemented', {
      evidenceRefs: ['src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/requestTabState.js', 'src/renderer/sessionPersistence.js', 'src/renderer/rendererWorkflows.js', 'electron/runtimeIpc.js', 'electron/main.js', 'src/core/runtimeResultStore.js', 'src/core/resultCapturePolicy.js'],
    ...
    row('workflow.performance', 'Performance', 'The Performance section exposes local workspace-owned saved tests in the sidebar between Runners and History, empty-pane creation, New > Performance Test creation, import-request and manual request-entry paths, eight local modes with type-scoped input fields including one-click Full Endpoint Diagnosis with Quick/Medium/Extended scopes, capture settings, pre-run result-store size warnings, dirty performance tabs, save/discard semantics, type-specific safety-cap rejection, cancellation, target-rate cancellable local calibration with phase progress, bounded probe-and-verification stages, local latency-knee rejection, store-backed paged high-volume results up to one million requests, UAT-ready diagnostic CSV plus self-contained HTML result export, and local result panes without claiming distributed/cloud execution.', 'implemented', {
      evidenceRefs: ['src/renderer/index.html', 'src/renderer/renderer.js', 'src/renderer/performanceTestModel.js', 'src/core/performanceCalibration.js', 'src/core/runtimeResultStore.js', 'src/core/resultCapturePolicy.js', 'electron/main.js', 'docs/ARCHITECTURE.md', 'docs/TECH_SPECS.md'],
