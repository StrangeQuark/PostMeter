...
function buildProductionReadinessMatrix() {
  const rows = [
    ...
    row('compatibility.non-postman', 'compatibility', 'OpenAPI, curl, and native PostMeter import/export compatibility are matrixed and validated.', 'validated', {
      ...
      notes: 'The source-owned non-Postman matrix has 7 implemented rows covering OpenAPI, curl, native PostMeter round trips, invalid input behavior, cross-shell quoting, and explicit preserve-only claim boundaries. Focused format tests exercise OpenAPI local references, server variables, path/query/header/cookie params, cookie API-key security, Swagger 2.0 body/form-data import, binary body hints, and response examples; curl auth, redirects, compression, repeated/query data, binary/file intent, generated names, and Windows-style line-continuation quoting; plus native workspace/Postman durability coverage for complex workspace features.'
    }),
