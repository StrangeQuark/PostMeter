...
function buildProductionReadinessMatrix() {
  const rows = [
    ...
    row('updates.metadata', 'release', 'Release/update metadata and opt-in automatic update wiring point to GitHub Releases without introducing a cloud account gate.', 'validated', {
      releaseBlocking: true,
    ...
    row('load.distributed', 'deferred-features', 'Distributed/cloud load execution is deferred and not part of the local production claim.', 'deferred', {
      releaseBlocking: false,
      commands: [],
      evidenceRefs: ['docs/TECH_SPECS.md', 'docs/COMPATIBILITY.md']
    })
  ];
