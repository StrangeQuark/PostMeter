const STATUS_DESCRIPTIONS = Object.freeze({
  ...
  'external-validation-required': 'Implementation exists, but final evidence requires native runners, provider signoff, or optional sanitized artifacts.',
...
const RELEASE_LEVEL_POLICIES = Object.freeze({
  ...
  stable: {
    allowedReleaseBlockingStatuses: ['validated'],
    allowDocumentedWaivers: false,
    description: 'Stable readiness requires every release-blocking row to be validated with required local/native/provider evidence.'
  }
});
...
function buildProductionReadinessMatrix() {
  const rows = [
    ...
    row('sandbox.platform-os', 'security', 'Platform-equivalent OS sandbox claim is implemented and tracked separately from Postman API parity.', 'validated', {
      releaseBlocking: true,
      commands: ['npm run sandbox:platform:validate', 'npm run sandbox:platform:claim'],
      evidenceRefs: ['docs/os-sandbox-platform-matrix.json', 'src/core/sandbox/osSandboxPlatformMatrix.js', 'native/windows-sandbox-helper/PostMeterWindowsSandboxHelper.cpp'],
    ...
    row('grpc.pfx-p12-mtls', 'transport', 'HTTP and gRPC mTLS share parent-owned in-memory PFX/P12 extraction aligned with client-certificate handling.', 'validated', {
      releaseBlocking: true,
      commands: ['npm test'],
      evidenceRefs: ['src/core/http/pfxCertificate.js', 'src/core/http/grpcClient.js', 'src/core/http/httpClient.js', 'src/core/runtime/scriptedRequestLifecycle.js', 'src/core/sandbox/scriptSandbox.js', 'src/core/import-export/postmanImporter.js', 'test/electron/grpcClient.test.js', 'test/electron/httpClient.test.js', 'test/electron/scriptSandbox.test.js', 'test/electron/scriptedRequestLifecycle.test.js', 'test/electron/postmanImporter.test.js'],
    ...
    row('vault.per-request-prompts', 'security', 'Per-request vault prompt decisions are brokered without exposing vault secrets to scripts or renderer APIs.', 'validated', {
      releaseBlocking: true,
      commands: ['npm test'],
      evidenceRefs: ['src/core/sandbox/scriptSandbox.js', 'src/core/runtime/scriptedRequestLifecycle.js', 'src/core/runtime/collectionRunner.js', 'src/core/workspace/models.js', 'src/core/workspace/workspaceMigrations.js', 'electron/ipc/vaultPrompt.js', 'electron/ipc/requestIpc.js', 'electron/ipc/runtimeIpc.js', 'src/renderer/index.html', 'src/renderer/app/rendererBootstrap.js', 'src/renderer/ui/vaultPromptQueue.js', 'docs/TROUBLESHOOTING.md', 'test/electron/scriptSandbox.test.js', 'test/electron/collectionRunner.test.js', 'test/electron/runnerModel.test.js', 'test/electron/runtimeIpc.test.js', 'test/electron/vaultPrompt.test.js', 'test/electron/vaultPromptQueue.test.js', 'test/electron/rendererBootstrap.test.js', 'test/electron/workspaceStore.test.js'],
    ...
    row('postman.newman-json-reports', 'compatibility', 'Deterministic Newman JSON evidence reports for approved differential fixtures are checked in and validated.', 'validated', {
      releaseBlocking: true,
    ...
    row('electron.security', 'security', 'Electron BrowserWindow, main-frame-only preload, channel-level IPC, IPC sender validation, navigation, webview, permissions, file dialogs, external URL, protocol, CSP, fuse-review, and packaged-preload controls are matrixed and validated.', 'validated', {
      releaseBlocking: true,
      commands: ['npm run electron:security:validate', 'npm test'],
      evidenceRefs: ['docs/electron-security-matrix.json', 'src/core/diagnostics-release/productionSupportMatrices.js', 'electron/app-shell/appProtocol.js', 'electron/app-shell/mainWindow.js', 'electron/security/ipcSecurity.js', 'electron/app-shell/preload.js', 'electron/ipc/workspaceIpc.js', 'electron/ipc/appIpc.js', 'electron/services/oauthFlows.js', 'test/electron/appChrome.test.js'],
    ...
    row('workspace.durability', 'data-durability', 'Workspace persistence, migrations, recovery, vault metadata, package cache, and large-workspace budgets are matrixed and validated.', 'validated', {
      ...
      evidenceRefs: [
        ...
        'src/core/diagnostics-release/productionSupportMatrices.js',
        'src/core/workspace/workspacePersistence.js',
        'src/core/workspace/workspaceStore.js',
        'src/core/workspace/workspaceManager.js',
        'src/core/sandbox/vaultStore.js',
    ...
    row('compatibility.non-postman', 'compatibility', 'OpenAPI, curl, and native PostMeter import/export compatibility are matrixed and validated.', 'validated', {
      ...
      evidenceRefs: [
        ...
        'src/core/import-export/openApiFormats.js',
        'src/core/import-export/curlFormats.js',
    ...
    row('performance.local-v1', 'performance', 'Local first-class saved Performance tests are implemented as workspace-owned performanceTests with local execution, IPC, diagnostics boundaries, safety caps, import/export validation, scoped one-click Full Endpoint Diagnosis, and eight-type coverage.', 'implemented', {
      ...
      notes: 'This row tracks the local desktop Performance implementation and compatibility boundary.'
    }),
  ...
  return {
    schemaVersion: 1,
    generatedFrom: 'src/core/diagnostics-release/productionReadinessMatrix.js',
    deterministic: true,
...
function validateProductionReadinessMatrix(matrix) {
  ...
  if (matrix.generatedFrom !== 'src/core/diagnostics-release/productionReadinessMatrix.js') {
    errors.push('Production readiness matrix generatedFrom must be src/core/diagnostics-release/productionReadinessMatrix.js.');
  }
