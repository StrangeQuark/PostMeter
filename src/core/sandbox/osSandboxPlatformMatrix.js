...
const SOURCES = Object.freeze({
  ...
  osSandbox: {
    ...
    path: 'src/core/sandbox/osSandbox.js'
  },
  seccompPolicy: {
    title: 'Linux seccomp syscall policy',
    path: 'src/core/sandbox/seccompPolicy.js'
  },
  runtimeValidation: {
    title: 'Sandbox runtime validation',
    path: 'src/core/sandbox/sandboxRuntimeValidation.js'
  },
  releaseGate: {
...
function buildOsSandboxPlatformMatrix() {
  const rows = [
    ...
    row('windows.packaged-os-sandbox-validation', 'windows', 'packaged-validation', 'Packaged Windows artifacts validate the native helper is included as an extra resource and that AppContainer helper launch, Node permission flags, packaged path behavior, and script-worker execution work on a Windows runner.', 'implemented', {
      ...
      sourceRefs: ['sandboxContract', 'runtimeValidation', 'ciWorkflow', 'releaseValidationWorkflow', 'releaseWorkflow'],
      verificationRefs: [
    ...
    row('macos.seatbelt-backend', 'macos', 'native-backend', 'macOS script workers launch through the system seatbelt sandbox-exec backend with a deny-default profile, denied network access, explicit process-exec/process-fork denial, no broad process allowance, minimal environment, private writable temp directory, and read-only runtime/app/library access.', 'implemented', {
      ...
      sourceRefs: ['sandboxContract', 'osSandbox', 'runtimeValidation', 'ciWorkflow', 'releaseWorkflow'],
      verificationRefs: [
        'macos-seatbelt launcher path in src/core/sandbox/osSandbox.js',
    ...
    row('release-gate.platform-matrix-validation', 'all', 'release-gating', 'The OS-sandbox platform matrix is generated, committed, validated, and wired into check, CI, release workflow, and release-gate manifest validation.', 'validation-hook', {
      ...
      sourceRefs: ['releaseGate', 'ciWorkflow', 'releaseWorkflow'],
      verificationRefs: [
  ...
  return {
    schemaVersion: 1,
    generatedFrom: 'src/core/sandbox/osSandboxPlatformMatrix.js',
    deterministic: true,
    target: {
...
function validateOsSandboxPlatformMatrix(matrix) {
  ...
  if (matrix.generatedFrom !== 'src/core/sandbox/osSandboxPlatformMatrix.js') {
    errors.push('OS sandbox platform matrix generatedFrom must be src/core/sandbox/osSandboxPlatformMatrix.js.');
  }
