...
const { BODY_TYPES } = require('../workspace/models');
const {
  OS_SANDBOX_MODES,
...
const { buildUrl, sendRequest } = require('../http/httpClient');
const { cookieFromHeader, cookiesForRequest } = require('../http/cookieJar');
const { normalizeCookieDomain } = require('../http/cookieModel');
const { runtimeEnvironment } = require('../workspace/variableScope');
const {
...
const { normalizeTlsSettings } = require('../http/tlsSettings');
...
const SCRIPT_WORKER_ALLOWED_FILES = [
  ...
  path.join(__dirname, '..', 'workspace', 'dynamicVariables.js'),
  path.join(__dirname, '..', 'workspace', 'variableScope.js')
];
