...
Performance runs use the same reusable temp result database and expose Capture Settings for response bodies, script output, local variables, response headers, and transport timings. CSV export is generated only when requested by streaming from the temp result store, so normal runs do not leave historical result files behind. PostMeter deletes the current temp result database and SQLite sidecars on app shutdown, which means unexported Runner/Performance results are intentionally session-local.
