...
- `npm run postman:parity:claim` is the full 1:1 Postman script compatibility gate for the supported default import profile. It now reports zero default-import blockers and must stay green before that claim is made.
...
- The Postman script compatibility claim is ready for the supported default import profile. Local production resource ceilings remain documented safety policy for host-exhaustion-sized payloads, and platform-equivalent OS sandbox gaps remain tracked separately by `docs/os-sandbox-platform-matrix.json`.
...
- Keep `npm run postman:parity:claim` passing as the Postman import surface evolves; the current default import profile has zero claim blockers.
