...
- Native Windows and macOS syscall-policy backends are not implemented yet; native helper binaries or native Node addons are acceptable for those AppContainer/seatbelt-style launchers when release-owned and packaged-validated. Linux has the `bubblewrap` OS isolation and seccomp syscall-policy layer described above, accepted as sufficient for the current Linux claim. These platform-security gaps are tracked by `docs/os-sandbox-platform-matrix.json`, not by the Postman API parity claim.
