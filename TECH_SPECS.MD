...
- HTTPS client certificate: loads certificate material in the main/core layer from local paths, supports PEM certificate/key pairs and PFX/P12 bundles, normalizes PFX/P12 bundles into in-memory PEM certificate/key buffers before transport, applies optional CA certificates and passphrases, and does not expose certificate contents to the renderer.
...
- Uses Node `https` request transport for HTTPS client-certificate requests so PEM certificate options can be applied. PFX/P12 bundles are extracted parent-side into PEM buffers first to avoid platform-specific Node/OpenSSL PFX parsing behavior.
...
- Supports explicitly granted `pm.vault` access through the parent broker, including workspace, collection, and request grants with explicit denial overrides. If no grant exists, desktop sends/runs can display a metadata-only renderer prompt with request, collection, operation, secret-key, deny, reset, request-grant, collection-grant, and workspace-grant choices; prompt decisions are persisted as scoped vault-grant metadata without exposing secret values, vault paths, encryption keys, or secret lists to the renderer. Desktop vault values are stored outside workspace JSON in per-workspace encrypted vault files, using Electron `safeStorage` only when OS-backed encryption is available and not the Linux `basic_text` backend.
...
- `npm run sandbox:platform:claim` is the platform-equivalent full OS sandbox gate. It is intentionally separate from Postman API parity and currently fails closed on native Windows/macOS backend and packaged-validation rows. Linux is no longer blocked on a deny-by-default seccomp decision because the maintainer accepted the current `bubblewrap` plus dangerous-syscall seccomp policy for the current Linux claim.
...
- Native Windows and macOS syscall-policy backends are not fully claim-ready yet. `src/core/osSandbox.js` now has a fail-closed Windows helper contract via `POSTMETER_WINDOWS_OS_SANDBOX_HELPER` and a macOS seatbelt-style `sandbox-exec` launcher path where available, but `npm run sandbox:platform:claim` remains blocked until native runner source and packaged probes prove AppContainer/restricted-token or seatbelt-equivalent behavior. Linux has the `bubblewrap` OS isolation and seccomp syscall-policy layer described above, accepted as sufficient for the current Linux claim. These platform-security gaps are tracked by `docs/os-sandbox-platform-matrix.json`, not by the Postman API parity claim.
- Load tests do not execute pre-request or test scripts; sandbox v1 keeps scripted load testing out of scope.
- Keep `npm run postman:parity:claim`, `npm run postman:docs:validate`, and a current `npm run postman:docs:live` sweep green before preserving the tracked Postman script compatibility claim. Local production resource ceilings remain documented safety policy for host-exhaustion-sized payloads, and platform-equivalent OS sandbox gaps remain tracked separately by `docs/os-sandbox-platform-matrix.json`.
...
- Completed native-runner packaged custom-scheme OAuth redirect registration scripts still need execution in GitHub release CI or the manual native release validation workflow for Windows and macOS.
...
- Add native Windows/macOS OS sandbox backends using acceptable native helpers/addons and packaged validation so `npm run sandbox:platform:claim` can pass. Linux's current `bubblewrap` namespace plus dangerous-syscall seccomp policy is accepted for the current Linux claim; a stricter deny-by-default seccomp-BPF allowlist is optional future hardening.
